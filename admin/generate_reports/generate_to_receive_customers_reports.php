...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100);
        // Title
        $this->Cell(80, 10, 'TO RECEIVE CUSTOMER ORDERS LIST DATA REPORT', 0, 0, 'C');
        $this->Ln(5);
...
$pdf->Cell(38, 8, 'TRANSACTION ID', 1);
$pdf->Cell(25, 8, 'CUSTOMER ID', 1);
$pdf->Cell(40, 8, 'CUSTOMER NAME', 1);
$pdf->Cell(20, 8, 'CONTACT', 1);
$pdf->Cell(155, 8, 'ADDRESS', 1);
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $pdf->Cell(38, 8, $row['transaction_id'], 1);
    $pdf->Cell(25, 8, $row['customer_id'], 1);
    $pdf->Cell(40, 8, $row['customer_name'], 1);
    $pdf->Cell(20, 8, $row['customer_no'], 1);
    $pdf->Cell(155, 8, $row["address"] . ' ' . $row['municipality_name'] . ' ' . $row['province_name'] . ' ' . $row['region_name'] . ' ' . $row['zip'], 1);
}
$pdf->Output();
