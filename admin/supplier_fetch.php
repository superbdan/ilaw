...
foreach($result as $row)
{
	...
	$sub_array[] = $status;
	$sub_array[] = '<center><button type="button" name="change" id="'.$row["supplier_id"].'" class="btn btn-secondary btn-xs  change" data-toggle="tooltip" data-placement="bottom" title="Change Availability" data-status="'.$row["supplier_status"].'"> <i class="fa fa-adjust"></i></button> <button type="button" name="update" id="'.$row["supplier_id"].'" class="btn btn-primary btn-xs update" data-toggle="tooltip" data-placement="bottom" title="Edit Supplier"><i class="fa fa-pencil"></i></button></center>';
	$data[] = $sub_array;
}
